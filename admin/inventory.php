...
// Stock is now derived from stock_batches
// For tray products: count = number of active batches
// For others: sum of remaining
$stockExpr = "COALESCE((
    SELECT CASE
        WHEN p.unit = 'per tray' THEN COUNT(sb.id)
...
// Build full query with computed stock
$fullQuery = "
...
// Apply stock filters after computing
if ($filterStock === 'low') $where2 = " HAVING stock <= i_rl AND stock > 0";
elseif ($filterStock === 'out') $where2 = " HAVING stock = 0";
elseif ($filterStock === 'ok')  $where2 = " HAVING stock > i_rl";
else $where2 = "";

// Simpler approach: fetch all then filter in PHP for stock status
...
// Stats — compute from stock_batches
...
// Low stock: active batch count/sum <= reorder level and > 0
$r = $conn->query("
    ...
    AND (
        ...
        WHERE sb.product_id = i.product_id AND sb.status = 'active'
    ) <= i.reorder_level
    AND (
        SELECT CASE WHEN p2.unit='per tray' THEN COUNT(sb.id) ELSE SUM(sb.remaining) END
        FROM stock_batches sb JOIN products p2 ON p2.id = sb.product_id
...
$r = $conn->query("
    SELECT COALESCE(SUM(
        CASE WHEN p.unit = 'per tray'
             THEN (SELECT COUNT(*) FROM stock_batches sb WHERE sb.product_id = p.id AND sb.status = 'active')
             ELSE (SELECT COALESCE(SUM(remaining),0) FROM stock_batches sb WHERE sb.product_id = p.id AND sb.status = 'active')
        END
    ), 0) AS total
    FROM inventory i
    JOIN products p ON p.id = i.product_id
...
<body>
    <div class="admin-layout">
        ...
        <div class="main-content">
            ...
            <div class="table-wrapper">
                <?php
                // Fetch all rows and filter by stock status in PHP
                $rows = [];
                if ($inventory) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="product-cell">
                                            <div class="product-thumb" style="<?= !empty($inv['image_url']) ? 'padding:0;overflow:hidden;' : '' ?>">
                                                <?php if (!empty($inv['image_url'])): ?>
                                                    <img src="<?= htmlspecialchars($inv['image_url']) ?>" alt="<?= htmlspecialchars($inv['name']) ?>" style="width:100%;height:100%;object-fit:cover;border-radius:8px;">
                                                <?php else: ?>
                                                    <?= $emoji ?>
                                                <?php endif; ?>
                                            </div>
                                    ...
                                    <td>
                                        <div class="stock-wrap">
                                            ...
                                            <span class="stock-unit-label"><?= $unitLbl ?></span>
                                            <div class="stock-bar">
                                    ...
                                    <td style="text-align:center;">
                                        <div style="display:flex;align-items:center;justify-content:center;gap:6px;">
                                            <button class="btn-action btn-adjust"
                                                onclick="openAdjust(<?= htmlspecialchars(json_encode(['product_id' => $inv['product_id'], 'name' => $inv['name'], 'qty' => $qty, 'unit' => $inv['unit'], 'reorder' => $reord, 'is_tray' => $isTray]), ENT_QUOTES) ?>)">
                ...
                <div class="modal-body">
                    <div class="product-info-box">
                        ...
                        <div>
                            ...
                            <div class="product-info-stock">Current stock: <strong id="adj_current">0</strong> <span id="adj_unit_label">units</span></div>
                        </div>
                    </div>
                    <div class="type-tabs">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label class="form-label" id="adj_qty_label">Quantity <span class="req">*</span></label>
    ...
    <div class="modal-backdrop" id="logsModal" onclick="backdropClose(event,'logsModal')">
        <div class="modal-card" style="max-width:680px;">
            <div class="modal-header">
                <div class="modal-title"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M9 14l6 0M9 10l6 0M16 2H8a4 4 0 0 0-4 4v14l4-2 4 2 4-2 4 2V6a4 4 0 0 0-4-4z" />
                    </svg><span id="logs_title">Stock History</span></div>
    ...
    <script>
        ...
        let _adjIsTray = false;

        function openAdjust(data) {
            _adjIsTray = data.is_tray || false;
            document.getElementById('adj_product_id').value = data.product_id;
            document.getElementById('adj_name').textContent = data.name;
            document.getElementById('adj_current').textContent = data.qty;
            document.getElementById('adj_reorder').value = data.reorder;
            document.getElementById('adj_qty').value = '';
            const unitLbl = _adjIsTray ? (data.qty != 1 ? 'trays' : 'tray') : data.unit;
            document.getElementById('adj_unit_label').textContent = unitLbl;
            const qtyLbl = document.getElementById('adj_qty_label');
            qtyLbl.innerHTML = (_adjIsTray ? 'Trays' : 'Quantity') + ' <span class="req">*</span>';
            const isChicken = data.name.toLowerCase().includes('chicken') || (data.unit || '').toLowerCase().includes('alive') || (data.unit || '').toLowerCase().includes('process');
            document.getElementById('adj_emoji').innerHTML = isChicken ? '<i class="fa-solid fa-drumstick-bite"></i>' : '<i class="fa-solid fa-egg"></i>';
            setType('in');
        ...
        function setType(type) {
            ...
            const label = _adjIsTray ? 'trays' : 'units';
            const hints = {
                'in': label.charAt(0).toUpperCase() + label.slice(1) + ' to add to current stock',
                'out': label.charAt(0).toUpperCase() + label.slice(1) + ' to deduct from current stock',
                'adjustment': 'Set stock to this exact number of ' + label
            };
