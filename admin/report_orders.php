...
<head>
    ...
    <style>
        ...
        .cust-avatar {
            ...
            height: 30px;
            border-radius: 50%;
            background: linear-gradient(135deg, #3b7dd8, #3b7dd8);
            display: flex;
            align-items: center;
            justify-content: center;
            font-size: 0.75rem;
...
<body>
        ...
        <div class="main-content">
            ...
            <form method="GET" action="report_orders.php">
                <div class="filter-bar">
                    ...
                    <div class="quick-ranges">
                        ...
                        ?>
                            <a href="report_orders.php?from=<?= $f ?>&to=<?= $t ?><?= $per ? '&period=' . $per : '' ?>"
            ...
            <div class="kpi-grid">
                ...
                <div class="kpi-card kc-red">
                    ...
                    <div class="kpi-sub">
                        <?php if ($totalOrders > 0): ?>
                            <span class="hi" style="color:#d94f46;"><?= round(($cancelledOrders / $totalOrders) * 100) ?>%</span> cancellation rate
                        <?php else: ?>
                            No orders in range
                        <?php endif; ?>
                    </div>
            ...
            <div class="section-header">
                ...
                <div style="display:flex;gap:8px;">
                    <button class="btn-export" onclick="printReport()" style="background:var(--brand,#e67e22);color:#fff;border-color:var(--brand,#e67e22);">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="table-wrapper">
                ...
                $maxTotal = 1;
                foreach ($rows as $r) {
                    if ((float)$r['total_amount'] > $maxTotal) $maxTotal = (float)$r['total_amount'];
                }
                ?>
                <?php if (!empty($rows)): ?>
                    <table class="report-table" id="ordersTable">
                        ...
                        <tbody>
                            <?php foreach ($rows as $i => $o):
                                ...
                                $pmIcon    = $o['payment_method'] === 'gcash' ? '<i class="fa-solid fa-mobile-screen"></i>' : ($o['payment_method'] === 'cod' ? '<i class="fa-solid fa-truck"></i>' : '<i class="fa-solid fa-money-bill"></i>');
                                $pct       = $maxTotal > 0 ? round(((float)$o['total_amount'] / $maxTotal) * 100) : 0;
                                $barColor  = $o['status'] === 'cancelled' ? '#d94f46' : ($o['status'] === 'delivered' ? '#2f9e60' : '#3b7dd8');
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        <div class="conv-wrap">
                                            ...
                                            <div class="conv-track">
                                                <div class="conv-fill" style="width:<?= $pct ?>%;background:<?= $barColor ?>;"></div>
                                            </div>
                                        </div>
                                    </td>
                ...
                <?php else: ?>
                    <div class="empty-state">
                        <div style="font-size:2.5rem;margin-bottom:12px;"><i class="fa-solid fa-clipboard-list"></i></div>
    ...
    <script>
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: function(context) {
                            ...
                            if (!chart.chartArea) return '#3b7dd8';
                            const {
                                top,
                                bottom
                            } = chart.chartArea;
        ...
        // ── Print (opens clean print view with matching period) ──────
        function printReport() {
            ...
            window.open('report_orders_print.php?' + params.toString(), '_blank');
        }
