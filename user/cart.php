...
while ($row = $cartRows->fetch_assoc()) {
    ...
    if (strtolower($row['unit']) === 'per piece' && (int)$row['quantity'] < 12) {
        $hasPieceMinViolation = true;
    }
    $cartData[] = $row;
}
...
<head>
    ...
    <style>
        ...
        *::after {
        ...
        .cart-qty-control.loading .cart-qty-btn {
            pointer-events: none;
            opacity: 0.4
        }

        .cart-qty-control.loading .cart-qty-input {
            color: #aaa
        }
...
<body>
    <div class="page-body">
        ...
        <div class="container">
            <div class="cart-layout">
                ...
                <div>
                    <?php if ($hasPieceMinViolation): ?>
                        <div class="min-warning-banner">
                            <i class="fa-solid fa-triangle-exclamation"></i>
                            <div>
                                <strong>Minimum order not met.</strong> Eggs sold <em>per piece</em> require a minimum of <strong>12 pieces (1 dozen)</strong> per product. Please adjust the quantity before checking out.
                            </div>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="cart-card">
                        ...
                        <?php if (count($cartData) > 0): ?>
                            <?php foreach ($cartData as $item):
                                ...
                                $isPiece   = strtolower($item['unit']) === 'per piece';
                                $belowMin  = $isPiece && $qty < 12;
                            ?>
                                <div class="cart-item" id="cartItem<?= $item['cart_id'] ?>">
                                    <div class="cart-item-thumb <?= $thumbCls ?>">
                                    ...
                                    <div class="cart-item-info">
                                        ...
                                        <?php endif; ?>
                                        <?php if ($belowMin): ?>
                                            <div class="piece-min-warning">
                                                <i class="fa-solid fa-triangle-exclamation" style="color:#f59e0b;"></i>
                                                12 pieces minimum (1 dozen) — currently <?= $qty ?>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                ...
                <div class="summary-card">
                    ...
                    <div class="summary-body">
                        ...
                        <?php if ($hasPieceMinViolation): ?>
                            <div class="checkout-blocked">
                                <i class="fa-solid fa-triangle-exclamation" style="color:#f59e0b;"></i>
                                Some items require a minimum of 12 pieces. Adjust quantities above to proceed.
                            </div>
                        <?php endif; ?>

                        <button class="btn-checkout"
                            id="checkoutBtn"
                            <?= (count($cartData) === 0 || $hasPieceMinViolation) ? 'disabled' : '' ?>
                            onclick="location.href='checkout.php'">
                            Proceed to Checkout →
                        </button>
                        <button class="btn-continue" onclick="location.href='products.php'">← Continue Shopping</button>
                        <div class="trust-row">
    ...
    <script>
            ...
            fetch('cart_action.php', {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        document.getElementById('summaryTotal').textContent = '₱' + grand.toLocaleString('en-PH', {
        ...
        function checkAllMinimums() {
            let hasViolation = false;
            document.querySelectorAll('.cart-qty-control').forEach(ctrl => {
                const unit = ctrl.dataset.unit || '';
                const qty = parseInt(ctrl.dataset.qty) || 0;
                const cartId = ctrl.id.replace('qtyCtrl', '');
                const warningEl = document.getElementById('pieceWarn' + cartId);
                if (unit.toLowerCase() === 'per piece') {
                    if (qty < 12) {
                        hasViolation = true;
            ...
            const banner = document.getElementById('minViolationBanner');
            const blocked = document.getElementById('checkoutBlockedMsg');
            const btn = document.getElementById('checkoutBtn');
            if (banner) banner.style.display = hasViolation ? 'flex' : 'none';
            if (blocked) blocked.style.display = hasViolation ? 'flex' : 'none';
            if (btn) btn.disabled = hasViolation;
        }
        ...
        document.querySelectorAll('.min-warning-banner').forEach(el => el.id = 'minViolationBanner');
        document.querySelectorAll('.checkout-blocked').forEach(el => el.id = 'checkoutBlockedMsg');
    </script>
</body>
