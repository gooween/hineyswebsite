...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'upload_proof') {
    ...
    $chk = $conn->prepare("SELECT id, payment_method, payment_status, status FROM orders WHERE id=? AND user_id=? LIMIT 1");
...
if (isset($_GET['get_order']) && is_numeric($_GET['get_order'])) {
    ...
    // Include image_url for product thumbnails in modal
    $iStmt = $conn->prepare("
...
<head>
    ...
    <style>
        ...
        *::after {
        ...
        .proof-file-input-wrap input[type="file"] {
            position: absolute;
            inset: 0;
            opacity: 0;
            width: 100%;
                ...
                <?php if (count($orderList) > 0): ?>
                    <div class="orders-list">
                        <?php foreach ($orderList as $o):
                            ...
                            $canUploadProof = ($sid === 'approved' && $o['payment_method'] === 'gcash' && $o['payment_status'] === 'unpaid');
                                    ...
                                    <div class="proof-uploaded-section">
                                        ...
                                        <div class="proof-uploaded-info">
                                            <div class="proof-uploaded-title"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#065f46" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                                ...
                                                </svg> Payment proof uploaded</div>
                                        ...
                                        <form method="POST" action="orders.php" enctype="multipart/form-data" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                                            ...
                                            <div style="position:relative;overflow:hidden;">
                                                ...
                                                <input type="file" name="gcash_proof" accept="image/*" style="position:absolute;inset:0;opacity:0;width:100%;cursor:pointer;" onchange="this.closest('form').submit()" required>
                                            </div>
                                        </form>
                                    </div>
                                <?php endif; ?>
        ...
        <div class="modal" id="modalBox">
            ...
            <div class="modal-body" id="modalBody">
                <div class="modal-loading">
                    <div class="spinner-lg"></div>Loading…
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function viewOrder(orderId) {
            ...
            fetch('orders.php?get_order=' + orderId)
                ...
                .then(data => {
                    if (!data.success) {
                        document.getElementById('modalBody').innerHTML = '<div class="modal-loading">✕ Could not load.</div>';
                        return;
                    }
                    renderModal(data);
                })
                .catch(() => {
                    document.getElementById('modalBody').innerHTML = '<div class="modal-loading">✕ Network error.</div>';
                });
        }
        ...
        function renderModal(data) {
            ...
            const fmtPeso = v => '₱' + parseFloat(v).toLocaleString('en-PH', {
            ...
            data.items.forEach(item => {
                ...
                // Show product image if available, otherwise emoji
                const thumbHtml = item.image_url ?
                    `<img src="${esc(item.image_url)}" style="width:28px;height:28px;border-radius:6px;object-fit:cover;margin-right:6px;vertical-align:middle;">` :
                    `<span style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:6px;font-size:1rem;margin-right:6px;${isChick?'background:linear-gradient(135deg,#f0fdf4,#bbf7d0)':'background:linear-gradient(135deg,#fef9ee,#fdeec8)'}">${isChick?'<i class="fa-solid fa-drumstick-bite"></i>':'<i class="fa-solid fa-egg"></i>'}</span>`;
                itemRows += `<tr>
            <td style="vertical-align:middle;">${thumbHtml}${esc(item.name)}<div style="font-size:0.72rem;color:var(--text-muted);">${esc(item.unit)}</div></td>
