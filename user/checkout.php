...
// GCash flow: place order → admin approves → customer uploads proof
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            // 1. Insert order — always starts as unpaid pending
            $stmt = $conn->prepare("
                INSERT INTO orders
                    (user_id, status, total_amount, delivery_fee, payment_method,
            ...
            foreach ($cartData as $item) {
                ...
                $conn->query("UPDATE inventory SET quantity = quantity - {$qty}
                              WHERE product_id = {$pid} AND quantity >= {$qty}");

                $reason = "Order #{$orderId} — {$item['name']} x{$qty}";
                $stmt = $conn->prepare("
                    INSERT INTO inventory_logs (product_id, type, quantity, reason, created_by, created_at)
                    VALUES (?, 'out', ?, ?, ?, NOW())
                ");
...
// Recalculate based on selected delivery type (for display)
...
.checkout-steps { display:flex; align-items:center; gap:0; margin-top:20px; }
.step { display:flex; align-items:center; gap:8px; font-size:0.78rem; font-weight:600; }
.step-num { width:24px; height:24px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:0.7rem; font-weight:800; }
.step.done .step-num { background:var(--success); color:#fff; }
.step.active .step-num { background:var(--primary); color:#fff; }
.step.pending .step-num { background:rgba(255,255,255,0.15); color:#8fa3b3; }
.step.done .step-label { color:#6ee7b7; }
.step.active .step-label { color:#fff; font-weight:700; }
.step.pending .step-label { color:#6b7a99; }
.step-line { height:2px; width:32px; background:rgba(255,255,255,0.1); margin:0 4px; }
.step-line.done { background:var(--success); }

/* Layout */
.container { max-width:1200px; margin:0 auto; padding:0 32px; }
@media(max-width:600px) { .container { padding:0 16px; } }
.checkout-layout { display:grid; grid-template-columns:1fr 380px; gap:28px; padding:32px 0 60px; align-items:start; }
@media(max-width:960px) { .checkout-layout { grid-template-columns:1fr; } }

/* Form Cards */
.form-card { background:var(--card-bg); border:1px solid var(--border); border-radius:var(--radius); box-shadow:var(--shadow); margin-bottom:20px; overflow:hidden; }
.form-card-header { display:flex; align-items:center; gap:10px; padding:16px 22px; border-bottom:1px solid var(--border); background:#fafafa; }
.form-card-num { width:26px; height:26px; background:var(--primary); color:#fff; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:0.75rem; font-weight:800; flex-shrink:0; }
.form-card-title { font-size:0.95rem; font-weight:800; color:var(--dark2); }
...
.form-input:focus, .form-textarea:focus { border-color:var(--primary); box-shadow:0 0 0 3px rgba(230,126,34,0.1); background:#fff; }
.form-input[readonly] { background:#f3f4f6; color:var(--text-muted); cursor:not-allowed; }
...
.delivery-option:hover { border-color:rgba(230,126,34,0.4); background:var(--primary-light); }
.delivery-option.selected { border-color:var(--primary); background:var(--primary-light); }
.delivery-option input[type="radio"] { width:16px; height:16px; accent-color:var(--primary); flex-shrink:0; }
.delivery-option-icon { font-size:1.4rem; flex-shrink:0; }
.delivery-option-name { font-size:0.88rem; font-weight:700; color:var(--dark2); }
.delivery-option-desc { font-size:0.72rem; color:var(--text-muted); }
...
.payment-option:hover { border-color:rgba(230,126,34,0.4); background:var(--primary-light); }
.payment-option.selected { border-color:var(--primary); background:var(--primary-light); }
.payment-option input[type="radio"] { width:18px; height:18px; accent-color:var(--primary); margin-top:2px; flex-shrink:0; }
.payment-option-icon { font-size:1.5rem; flex-shrink:0; line-height:1; }
.payment-option-name { font-size:0.92rem; font-weight:700; color:var(--dark2); margin-bottom:2px; }
.payment-option-desc { font-size:0.78rem; color:var(--text-muted); line-height:1.5; }

/* GCash info box (read-only — no ref input needed at checkout) */
.gcash-info { display:none; margin-top:16px; padding:16px; background:#eff6ff; border:1.5px solid #bfdbfe; border-radius:10px; }
.gcash-info.show { display:block; }
.gcash-info-title { font-size:0.82rem; font-weight:700; color:#1e40af; margin-bottom:4px; display:flex; align-items:center; gap:6px; }
.gcash-info-note  { font-size:0.8rem; color:#3b82f6; line-height:1.6; margin-bottom:12px; }
.gcash-steps { display:flex; flex-direction:column; gap:8px; margin-bottom:0; }
.gcash-step { display:flex; align-items:flex-start; gap:10px; font-size:0.82rem; color:#1e40af; }
.gcash-step-num { width:22px; height:22px; background:#3b82f6; color:#fff; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:0.68rem; font-weight:800; flex-shrink:0; margin-top:1px; }

/* Pickup info */
.pickup-info { display:none; margin-top:12px; padding:14px; background:#f0fdf4; border:1.5px solid #86efac; border-radius:10px; }
.pickup-info.show { display:block; }
.pickup-info-title { font-size:0.82rem; font-weight:700; color:#065f46; margin-bottom:6px; display:flex; align-items:center; gap:6px; }
.pickup-info-address { font-size:0.85rem; color:#065f46; line-height:1.6; background:#fff; border:1px solid #86efac; border-radius:8px; padding:10px 14px; }

/* Error alert */
.alert-error { background:#fef2f2; border:1px solid #fecaca; border-left:4px solid var(--danger); border-radius:9px; padding:14px 16px; margin-bottom:20px; font-size:0.87rem; color:#991b1b; }
.alert-error ul { padding-left:18px; margin-top:6px; }
.alert-error li { margin-bottom:4px; }

/* Summary card */
.summary-card { background:var(--card-bg); border:1px solid var(--border); border-radius:var(--radius); box-shadow:var(--shadow); overflow:hidden; position:sticky; top:calc(var(--navbar-h) + 20px); }
.summary-header { padding:16px 22px; border-bottom:1px solid var(--border); font-size:0.95rem; font-weight:800; color:var(--dark2); display:flex; align-items:center; gap:8px; }
.summary-items { max-height:260px; overflow-y:auto; border-bottom:1px solid var(--border); scrollbar-width:thin; }
.summary-item { display:flex; align-items:center; gap:12px; padding:12px 18px; border-bottom:1px solid #f9fafb; font-size:0.84rem; }
.summary-item:last-child { border-bottom:none; }
.summary-item-thumb { width:38px; height:38px; border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:1.3rem; flex-shrink:0; }
.si-egg   { background:linear-gradient(135deg,#fef9ee,#fdeec8); }
.si-chick { background:linear-gradient(135deg,#f0fdf4,#bbf7d0); }
.summary-item-name { flex:1; font-weight:600; color:var(--dark2); line-height:1.3; }
.summary-item-qty  { font-size:0.75rem; color:var(--text-muted); }
.summary-item-price { font-weight:700; color:var(--primary); white-space:nowrap; }
.summary-totals { padding:16px 18px; }
.total-row { display:flex; justify-content:space-between; align-items:center; font-size:0.85rem; padding:6px 0; }
.total-row-label { color:var(--text-muted); }
.total-row-value { font-weight:600; color:var(--dark2); }
.total-divider { height:1px; background:var(--border); margin:10px 0; }
.grand-total-row { display:flex; justify-content:space-between; align-items:center; padding:8px 0 4px; }
.grand-total-label { font-size:1rem; font-weight:800; color:var(--dark2); }
.grand-total-value { font-size:1.4rem; font-weight:900; color:var(--primary); letter-spacing:-0.03em; }
.summary-footer { padding:16px 18px 20px; border-top:1px solid var(--border); }
.btn-place-order { display:flex; align-items:center; justify-content:center; gap:8px; width:100%; padding:14px; background:var(--primary); color:#fff; border:none; border-radius:10px; font-size:1rem; font-weight:800; cursor:pointer; font-family:inherit; transition:all var(--transition); }
.btn-place-order:hover { background:var(--primary-dark); transform:translateY(-1px); box-shadow:0 8px 24px rgba(230,126,34,0.4); }
.btn-place-order:disabled { background:#d1d5db; cursor:not-allowed; transform:none; box-shadow:none; }
.btn-back-cart { display:flex; align-items:center; justify-content:center; gap:6px; width:100%; padding:10px; background:transparent; border:1.5px solid var(--border); border-radius:10px; font-size:0.85rem; font-weight:600; cursor:pointer; font-family:inherit; color:var(--text-muted); margin-top:10px; transition:all var(--transition); }
.btn-back-cart:hover { border-color:var(--primary); color:var(--primary); background:var(--primary-light); }
.trust-row { display:flex; justify-content:center; gap:14px; margin-top:14px; flex-wrap:wrap; }
.trust-item { font-size:0.7rem; color:var(--text-muted); display:flex; align-items:center; gap:4px; }

/* Delivery fee badge */
.free-badge { display:inline-flex; align-items:center; gap:4px; background:#d1fae5; color:#065f46; font-size:0.72rem; font-weight:700; padding:2px 8px; border-radius:10px; }
.tbd-badge  { display:inline-flex; align-items:center; gap:4px; background:#fef3c7; color:#92400e; font-size:0.72rem; font-weight:700; padding:2px 8px; border-radius:10px; }
...
<div class="page-banner">
    <div class="page-banner-inner">
        ...
        <div class="checkout-steps">
            <div class="step done"><div class="step-num">✓</div><span class="step-label">Cart</span></div>
            <div class="step-line done"></div>
            <div class="step active"><div class="step-num">2</div><span class="step-label">Checkout</span></div>
            <div class="step-line"></div>
            <div class="step pending"><div class="step-num">3</div><span class="step-label">Confirmation</span></div>
        </div>
    </div>
</div>
...
<div class="checkout-layout">
    ...
    <div>
        ...
        <form method="POST" action="checkout.php" id="checkoutForm">
            ...
            <!-- 1. Contact Info -->
            <div class="form-card">
                <div class="form-card-header"><div class="form-card-num">1</div><div class="form-card-title">Contact Information</div></div>
            ...
            <!-- 2. Delivery / Pickup -->
            <div class="form-card">
                <div class="form-card-header"><div class="form-card-num">2</div><div class="form-card-title">Delivery Method</div></div>
                <div class="form-card-body">

                    <div class="delivery-toggle">
                        <label class="delivery-option <?= ($selectedDeliveryType !== 'pickup') ? 'selected' : '' ?>"
                    ...
                    <div id="deliveryAddressWrap">
                        <div class="form-group">
                            ...
                            <textarea name="delivery_address" id="deliveryAddress"
                                      class="form-textarea"
                                      placeholder="House/Unit No., Street, Barangay, Loreto Cortes, Bohol"><?= htmlspecialchars($_POST['delivery_address'] ?? $user['address'] ?? '') ?></textarea>
                    ...
                    <div class="form-group" style="margin-top:14px;">
                        ...
                        <textarea name="notes" class="form-textarea"
                                  placeholder="Special instructions, preferred pickup time, gate code…"><?= htmlspecialchars($_POST['notes'] ?? '') ?></textarea>
                    </div>
            ...
            <!-- 3. Payment -->
            <div class="form-card">
                <div class="form-card-header"><div class="form-card-num">3</div><div class="form-card-title">Payment Method</div></div>
                <div class="form-card-body">
                    ...
                    <!-- GCash info (shown when gcash selected — no input needed at this stage) -->
                    <div class="gcash-info <?= ($_POST['payment_method'] ?? '') === 'gcash' ? 'show' : '' ?>" id="gcashInfo">
                        <div class="gcash-info-title">
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                            How GCash Payment Works
                        </div>
                        <div class="gcash-info-note">
                            No need to pay yet! Here's the simple process:
                        </div>
        ...
        <div>
            <div class="summary-items">
                ...
                <div class="summary-item">
                    <div class="summary-item-thumb <?= $siCls ?>"><?= $emoji ?></div>
            ...
            <div class="summary-totals">
                ...
                <div class="total-row">
                    ...
                    <span class="total-row-value" id="summaryDeliveryFee">
                        <?php if ($selectedDeliveryType === 'pickup'): ?>
...
function selectDelivery(type) {
    ...
    if (radio) { radio.checked = true; radio.closest('.delivery-option').classList.add('selected'); }
...
function updateTotals(type) {
    ...
    const btnEl   = document.getElementById('btnTotal');
    const fmt     = v => '₱' + v.toLocaleString('en-PH', {minimumFractionDigits:2});

    feeEl.innerHTML = type === 'pickup'
        ? '<span class="free-badge">✓ FREE</span>'
        : fmt(DELIVERY_FEE);
    grandEl.textContent = fmt(grand);
    btnEl.textContent   = fmt(grand);
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.delivery-option').forEach(label => {
        label.addEventListener('click', function () {
            ...
            if (radio) selectDelivery(radio.value);
        });
    });
